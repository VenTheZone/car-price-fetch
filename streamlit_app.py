...
def fetch_autotrader(search_term):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results = []
            listings = soup.find_all('div', {'data-cmp': 'itemCard'})
            for listing in listings[:5]:
                price = listing.find('span', {'data-cmp': 'pricing'})
                title = listing.find('h2', {'data-cmp': 'title'})
                # Find image element - adjust selector based on actual website structure
...
def fetch_cargurus(search_term):
    """Fetch data from CarGurus"""
    try:
        url = f"https://www.cargurus.com/Cars/searchResults.action?sourceContext=homePageNewCar&searchContext=homePageNewCar&searchType=NEW&zip=&inventorySearchWidgetType=AUTO&nonShippableBaseline=0&sortDir=ASC&sourceContext=homePageNewCar&distance=50&sortType=DEAL_RATING_RPL&entitySelectingHelper.selectedEntity={search_term}"
        response = requests.get(url, headers=HEADERS, timeout=10)
        if response.status_code == 200:
            soup = BeautifulSoup(response.text, 'html.parser')
            results = []
            listings = soup.find_all('div', {'class': 'cardContent'})
            for listing in listings[:5]:
                price = listing.find('span', {'class': 'price'})
                title = listing.find('h4', {'class': 'title'})
                # Find image element - adjust selector based on actual website structure
                img = listing.find('img')
                img_url = img.get('src') if img else None
...
def fetch_cars_com(search_term):
    """Fetch data from Cars.com"""
    try:
        url = f"https://www.cars.com/shopping/results/?stock_type=all&makes[]={search_term}"
        response = requests.get(url, headers=HEADERS, timeout=10)
        if response.status_code == 200:
            soup = BeautifulSoup(response.text, 'html.parser')
            results = []
            listings = soup.find_all('div', {'class': 'vehicle-card'})
            for listing in listings[:5]:
                price = listing.find('span', {'class': 'primary-price'})
                title = listing.find('h2', {'class': 'title'})
                # Find image element - adjust selector based on actual website structure
                img = listing.find('img')
                img_url = img.get('src') if img else None
